...
module.exports = function(CONFIG){
	...
	module.upload = function(row,type,callback) {
		...
		if (!fs.existsSync(helper.check_config(CONFIG.TRANSMISSION.TORRENTS_DIR,sh.pwd()+"\\torrent_files"))) {
			fs.mkdirSync(helper.check_config(CONFIG.TRANSMISSION.TORRENTS_DIR,sh.pwd()+"\\torrent_files"));
		}
		...
		response_stream.on('response', function(response) {
			var torrent_file = helper.check_config(CONFIG.TRANSMISSION.TORRENTS_DIR,sh.pwd()+"\\torrent_files") + "\\" + f_name;
